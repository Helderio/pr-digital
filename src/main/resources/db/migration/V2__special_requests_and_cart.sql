...
CREATE INDEX IF NOT EXISTS idx_special_requests_user ON special_requests(user_id);
CREATE INDEX IF NOT EXISTS idx_special_requests_status ON special_requests(status);
CREATE INDEX IF NOT EXISTS idx_special_requests_store ON special_requests(store_id);

ALTER TABLE cart_items
    ADD COLUMN IF NOT EXISTS item_type VARCHAR(20) NOT NULL DEFAULT 'CATALOG',
    ADD COLUMN IF NOT EXISTS special_request_id UUID REFERENCES special_requests(id);

ALTER TABLE cart_items ALTER COLUMN product_id DROP NOT NULL;

ALTER TABLE cart_items DROP CONSTRAINT IF EXISTS cart_items_product_or_special_chk;
